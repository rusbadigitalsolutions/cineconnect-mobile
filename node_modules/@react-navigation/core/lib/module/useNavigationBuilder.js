 ...
 */
const getRouteConfigsFromChildren = (children, groupKeys = [], groupOptions, groupLayout) => {
  const configs = React.Children.toArray(children).reduce((acc, child) => {
    if (/*#__PURE__*/React.isValidElement(child)) {
      ...
      if (isGroup(child)) {
        ...
        acc.push(...getRouteConfigsFromChildren(child.props.children, child.props.navigationKey != null ? [...groupKeys, child.props.navigationKey] : groupKeys,
        // FIXME
...
const getStateFromParams = (params, type) => {
  const state = params?.state;
  if (isNavigationState(state)) {
    if (state.type != null && state.type !== type) {
      if (process.env.NODE_ENV !== 'production') {
        console.error(`The state passed in the params is for a navigator of type '${state.type}', but it was passed to a navigator of type '${type}'. This can happen if the state was meant for a different navigator. The state will be discarded.`);
      }
      return undefined;
    }
    return state;
...
export function useNavigationBuilder(createRouter, options) {
  ...
  const [stateBeforeInitialization, initializedState, isFirstStateInitialization, paramsUsedForInitialization] = React.useMemo(() => {
    ...
    if ((currentState === undefined || !isStateValid(currentState)) && route?.params?.state == null && !(typeof route?.params?.screen === 'string' && route?.params?.initial !== false) && !isNestedParamsConsumed) {
      ...
      const stateFromParams = paramsForState ? getStateFromParams(paramsForState, router.type) : undefined;
      ...
      if (stateBeforeInitialization != null && isStateValid(stateBeforeInitialization) && options.UNSTABLE_routeNamesChangeBehavior === 'lastUnhandled' && doesStateHaveOnlyInvalidRoutes(stateBeforeInitialization)) {
  ...
  const routeKeyChanges = Object.keys(routeKeyList).filter(name => {
    ...
    return current != null && previous != null && !isArrayEqual(current, previous);
  });
  ...
  unhandledState?.routes.every(r => routeNames.includes(r.name)) && state?.routes.every(r => !routeNames.includes(r.name))) {
  ...
  } else if (!isArrayEqual(state.routeNames, routeNames) || routeKeyChanges.length !== 0) {
  ...
  if (route?.params && !didConsumeNestedParams) {
    ...
    if (process.env.NODE_ENV !== 'production' && action != null && updatedState === null) {
      const names = routeNames.map(name => `'${name}'`).join(', ');
      console.error(`The 'state' or 'screen' passed in the params couldn't be applied to the navigator with the screens: ${names}. This can happen if the params contain names of screens that don't exist in the navigator. The navigation specified in the params will be ignored.`);
    }
